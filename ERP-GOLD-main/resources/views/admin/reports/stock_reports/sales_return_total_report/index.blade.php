                                   ...
                                   style="text-align: center;direction: rtl;">
                                    ...
                                    @foreach($sales_return??[] as $sale_return)
                                        <tr>
                                            ...
                                            <a href="{{route('sales_return.show' , $sale_return -> id)}}" target="_blank">{{$sale_return -> bill_number}}</a>
                                            </td>
                                            <td class="text-center">{{ \Carbon\Carbon::parse($sale_return -> date) -> format('d-m-Y')  }}</td>
                                            <td class="text-center">{{$sale_return -> customer_name}}</td>
                                            <td class="text-center">{{abs($sale_return -> total_quantity)}}</td> 
                                            <td class="text-center">{{$sale_return -> round_net_total}}</td>
                                            <td class="text-center">{{round($sale_return -> lines_total_after_discount, 2)}}</td> 
                                            <td class="text-center">{{round($sale_return -> taxes_total, 2)}}</td>  
                                        </tr>

                                        <?php $sum_weight += abs($sale_return->total_quantity) ?>
                                        <?php
                                        $sum_total += ($sale_return->lines_total_after_discount)
                                        ?>
                                        <?php $sum_tax += $sale_return->taxes_total ?>
                                        <?php $sum_net += $sale_return->net_total ?>
                                        <?php $sum_discount += $sale_return->discount ?>
                                    @endforeach 
                                    <tfoot>  
                                ...
                                <div class="row"> 
                                    <div class="table-responsive hoverable-table" style="direction: rtl;"> 
                                        ...
                                        <table class="table table-bordered"  width="100%" cellspacing="0">
                                            ...
                                            <tbody>
                                                @foreach($sales_return_by_carat as $sale_return_by_carat)
                                                <tr>
                                                    <td class="text-center">{{$loop->iteration}}</td>
                                                    <td class="text-center">{{$sale_return_by_carat->carat_title}}</td>
                                                    <td class="text-center">{{$sale_return_by_carat->total_quantity}}</td>
                                                    <td class="text-center">{{$sale_return_by_carat->total_weight}}</td>
                                                    <td class="text-center">{{round($sale_return_by_carat->total_line_total, 2)}}</td>
                                                    <td class="text-center">{{round($sale_return_by_carat->total_taxes_total, 2)}}</td>
                                                    <td class="text-center">{{'0'}}</td>
                                                    <td class="text-center">{{round($sale_return_by_carat->total_net_total, 2)}}</td>
                                                </tr>
                                                @endforeach        
                                            </tbody> 
                                        </table>
