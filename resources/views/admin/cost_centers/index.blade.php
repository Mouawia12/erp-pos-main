                ...
                <div class="card">
                    <div class="card-header pb-0">
                        <div class="col-lg-12 margin-tb">
                            <h4 class="alert alert-primary text-center">
                                {{ __('main.cost_centers') ?? 'مراكز التكلفة' }}
                            </h4>
                        </div>
                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="table-responsive hoverable-table">
                            <table class="display w-100 text-nowrap table-bordered" id="example1" style="text-align: center;">
                                <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{__('main.code')}}</th>
                                ...
                                @foreach($centers as $center)
                                    <tr>
                                        ...
                                        <td class="text-center">{{$center->code ?? '-'}}</td>
                                        <td class="text-center">{{$center->name}}</td>
                                        <td>
                                            <input type="checkbox" name="status[]" @if($center->is_active) checked @endif value="{{ $center->is_active }}">
                                        </td>
                                        <td class="text-center">
                                            @can('تعديل الاعدادات')
                                            <button type="button" class="btn btn-labeled btn-info" onclick="EditModal({{$center->id}})">
                                                <i class="fa fa-pen"></i>
                                            </button>
                                            @endcan
                                            @can('حذف الاعدادات')
                                            <button type="button" class="btn btn-labeled btn-danger deleteBtn" id="{{$center->id}}">
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <label class="modelTitle">{{ __('main.cost_centers') ?? 'مراكز التكلفة' }}</label>
                    <button type="button" class="close modal-close-btn" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" id="paymentBody">
                    <form method="POST" action="{{ route('cost_centers.store') }}" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="code" name="code"
                                           ...
                                           placeholder="{{ __('main.code') }}" value="{{ old('code') }}"/>
                                    @error('code')
...
<script type="text/javascript">
    ...
    function confirmDelete() {
        let url = "{{ route('cost_centers.delete', ':id') }}";
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response) {
                    ...
                    $(".modal-body #is_active").val(response.is_active ? 1 : 0);
