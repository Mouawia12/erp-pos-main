...
<!-- Navbar -->
@include('layouts.nav' , ['page_title' => __('main.deductions'). ' / '. __('main.update_deduction')])
<!-- End Navbar -->
                            ...
                            <div class="card m-b-30 col-md-4">
                                ...
                                <div class="card-body">
                                    <div class="input-group">
                                        <input type="text" id="default-date" required name="date" class="datepicker-here form-control" placeholder="dd/mm/yyyy" aria-describedby="basic-addon2"
                                    ...
                                    <div class="form-group mb-0">
                                        <select class="select2-single form-control" id="employer_id" name="employer_id">
                                            @foreach($employers as $employer)
                                                <option value="{{$employer->id}}" @if($employer->id == $deduction->employer_id) selected @endif>{{$employer->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="card-body">
                                    <div class="form-group mb-0">
                                        <input type="number" step="0.01"
                                               class="form-control" required name="amount" id="amount" placeholder="{{__('main.Amount')}}" value="{{$deduction->amount}}">
                                    </div>
                                </div>
                            </div>

                            <div class="card m-b-30 col-md-8">
                                <div class="card-header">
                                    <h5 class="card-title">{{__('main.reason')}}</h5>
                                </div>
                                <div class="card-body">
                                    <div class="form-group mb-0">
                                        <textarea class="form-control" name="reason" aria-label="With textarea">{{$deduction->reason}}</textarea>
                                    </div>
                                </div>
                            </div>
