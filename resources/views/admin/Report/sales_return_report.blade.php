                ...
                <div class="card-header pb-0">
                    <div class="col-lg-12 margin-tb">
                        <h4  class="alert alert-primary text-center">
                            {{__('main.sales_return_report')}}
                        </h4>
                    </div>
                    <div class="clearfix"></div> 
                </div> 
                <div class="card-body"> 
                    <form id="sales-return-report-form">
                    <div class="row">
                        <div class="col-md-6">
                       ...
                       <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-12 text-center">
                                ...
                                <span id="sales-return-pdf-spinner" class="pdf-spinner d-none" aria-hidden="true"></span>
                            </div>
                        </div> 
                    </form>
                    </div>
...
<div class="modal fade" id="salesReturnReportModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document" style="max-width: 95%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('main.sales_return_report') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <iframe id="sales-return-pdf-viewer"
                        src=""
                        style="width:100%; height:80vh; border:none;"></iframe>
                <div id="sales-return-pdf-error" class="alert alert-danger mt-3 d-none"></div>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#excute').click(async function (){
            ...
            await fetchSalesReturnReportPdf({
                from_date: fromDate,
                to_date: toDate,
    ...
    function showSalesReturnPdfError(message) {
        const errorBox = document.getElementById('sales-return-pdf-error');
        errorBox.textContent = message;
        errorBox.classList.remove('d-none');
    }

    function clearSalesReturnPdfError() {
        const errorBox = document.getElementById('sales-return-pdf-error');
        errorBox.textContent = '';
        errorBox.classList.add('d-none');
    }

    function setSalesReturnPdfLoading(isLoading) {
        const spinner = document.getElementById('sales-return-pdf-spinner');
    ...
    async function fetchSalesReturnReportPdf(params) {
        clearSalesReturnPdfError();
        setSalesReturnPdfLoading(true);
        try {
            const query = new URLSearchParams(params);
            const response = await fetch("{{ route('sales_return_report_pdf') }}?" + query.toString(), {
            ...
            if (window.salesReturnReportBlobUrl) {
                URL.revokeObjectURL(window.salesReturnReportBlobUrl);
            }
            const blobUrl = URL.createObjectURL(blob);
            window.salesReturnReportBlobUrl = blobUrl;

            const viewer = document.getElementById('sales-return-pdf-viewer');
            viewer.src = blobUrl;
            $('#salesReturnReportModal').modal('show');
        } catch (error) {
            showSalesReturnPdfError(error.message);
        } finally {
            setSalesReturnPdfLoading(false);
        }
    }

    $('#salesReturnReportModal').on('hidden.bs.modal', function () {
        if (window.salesReturnReportBlobUrl) {
            URL.revokeObjectURL(window.salesReturnReportBlobUrl);
            window.salesReturnReportBlobUrl = null;
        }
        const viewer = document.getElementById('sales-return-pdf-viewer');
        if (viewer) {
            viewer.src = '';
