...
<head>
    <meta charset="utf-8">
    <title>{{ __('main.sales_report_by_item') ?? 'تقرير المبيعات تفصيلي' }}</title>
    <style>
        @font-face {
            font-family: 'Tajawal';
...
<body>
    <div class="header">
        <h2>{{ __('main.sales_report_by_item') ?? 'تقرير المبيعات تفصيلي' }}</h2>
    ...
    <div class="meta">
        ...
        <div class="meta-row">
            <div class="meta-cell">{{ __('main.product_name') }}: {{ $item?->name ?? __('main.all') }}</div>
        ...
        <tbody>
            @forelse($rows as $detail)
                <tr>
                    ...
                    <td>{{ \Carbon\Carbon::parse($detail->created_at ?? $detail->bill_date)->format('Y-m-d H:i:s') }}</td>
                    ...
                    <td>{{ number_format(($detail->tax ?? 0) + ($detail->tax_excise ?? 0), 2) }}</td>
                    <td>{{ number_format(($detail->total ?? 0) + ($detail->tax ?? 0) + ($detail->tax_excise ?? 0), 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="14">{{ __('main.no_data') }}</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
