...
- [x] نسخ احتياطي للنظام/قاعدة البيانات.
- [⚠️] إعدادات النظام الموحدة: ترقيم تلقائي/يدوي للأصناف/الأقسام/المجموعات/الماركات، تنبيهات تلقائية (انتهاء كمية أو تاريخ صنف أو وثائق مندوب)، إعدادات الطباعة (A4/A5/كاشير)، إعدادات الحسابات والمخزون، وضع نقاط البيع (لمس/عادي)، الصلاحيات والمستخدمين. (تم إضافة نوع فاتورة افتراضي/تسجيل جهاز واحد، تقارير انتهاء/انخفاض المخزون متاحة، باقي البنود قيد العمل)
    - [⚠️] تنبيهات انتهاء تاريخ صنف: تقرير أصناف منتهية/قريبة الانتهاء (فلترة بالتاريخ/الفرع/المستودع؛ تنبيه آلي ما زال متبقياً).
- [x] خيار تقييد دخول المستخدم من جهاز واحد أو السماح بعدة أجهزة.
- [x] تسلسل فواتير/مستندات مستقل لكل مستخدم. (مُفعّل باختيار per_user_sequence في إعدادات النظام)
- [x] عرض بيانات الفروع (سجل تجاري، رقم ضريبي، عنوان، تواصل، مسؤول الفرع). (موجودة في بطاقة الفرع وتظهر في طباعة الفواتير)
- [x] في الإقرار الضريبي: عند اختيار جميع الفروع مع أرقام ضريبية مختلفة يجب اختيار الرقم الضريبي تجنباً لخلط الضرائب.
- [x] المصروفات: حقل ضريبة مع ترحيل تلقائي للضريبة إلى حساب الضريبة.
- [⚠️] وضع Offline مؤقت لتخزين العمليات محلياً (خصوصاً فواتير المبيعات) ومزامنتها عند عودة الاتصال. (تمت إضافة حفظ فواتير POS/المبيعات عند انقطاع الاتصال ومزامنتها تلقائياً عند العودة؛ مزامنة شاملة/خلفية تحتاج استكمال)
- [x] تذييل حقوق: "جميع الحقوق محفوظة لمؤسسة فن الاحتراف" مع سنة محدثة تلقائياً.
...
## 3) كروت الإدخال
### العملاء
- [x] حقل السجل التجاري الرئيسي.
- [x] كيانات فرعية تحت نفس الرقم الضريبي (حقل ربط بالكيان الرئيسي + سجل/رقم ضريبي).
- [x] ربط العميل بمستوى سعر محدد وافتراضي خصم.
- [x] ربط العميل بمندوب. (اختيار مندوب للعميل، ويتم تعيينه تلقائياً عند اختيار العميل في المبيعات/المشتريات/POS)
...
### الأصناف
- [x] وحدات متعددة ابتداءً بالأصغر لضمان الجرد (حبة/درزن/علبة/كرتون...) (إدارة وحدات وسعر ومعامل تحويل وباركود لكل وحدة).
- [ ] شجرة تصنيفات غير محدودة المستويات.
- [⚠️] 6+ أسعار بيع (قطاعي، جملة، عملاء خاصون، مناديب...) (تم إضافة price_level_1..6 وتطبيقها تلقائياً حسب مستوى سعر العميل؛ إدارة الأسعار لكل منتج ما زالت مطلوبة).
- [ ] خيار أسعار شاملة ضريبة القيمة المضافة (تفعيل/إلغاء).
- [ ] ضرائب متعددة للصنف (VAT + ضريبة انتقائية تطبق على السعر الصافي قبل VAT).
- [⚠️] أرقام تشغيل/تشغيلات (Batch/Lot) مع تاريخ وتشغيلات متعددة (تمت إضافة إدخال رقم التشغيل/تاريخ الانتهاء في المشتريات، دون إدارة متقدمة للبحث/التقارير بعد).
- [x] باركود يدوي/قارئ مع توليد باركود، وباركود مستقل لكل وحدة (حقول باركود لكل وحدة مع زر توليد سريع).
...
- [x] عرض مواقع تواجد الصنف (مستودعات، كميات، تكلفة، آخر سعر بيع).
...
### فواتير المبيعات
- [x] إظهار تكلفة المنتج + آخر سعر بيع + الكمية المتوفرة في شاشة الفاتورة.
- [x] اختيار الوحدة المباعة مع تحذير عند تكرار الصنف (تم حفظ معامل التحويل في الفاتورة/المرتجع والتطبيق على الخصم من المخزون والأرباح).
- [⚠️] أنواع دفع: أجل (عميل إلزامي)، نقدي، شبكة، أو مزيج مع عدة بنوك للشبكة (تم دعم مزيج نقد/عدة بطاقات، ربط محطات الشبكة المسبقة قيد المتابعة).
- [x] مركز تكلفة (عميل + مندوب) قابل للإضافة快捷. (تم إضافة اختيار مندوب لكل فاتورة مبيعات/مشتريات مع مركز تكلفة اختياري ويُعبّأ تلقائياً باسم المندوب عند اختياره)
- [x] احتساب الضريبة شامل/غير شامل. (يُحفظ وضع الضريبة وتُعاد احتساب الإجماليات في السيرفر ويُعرض في الطباعة A4/POS)
- [x] حقل ملاحظات.
- [x] عند اختيار عميل افتراضي تظهر حقول اسم العميل ورقم الجوال (حقلان اختياريان مع تفعيل عميل نقدي/افتراضي).
- [x] نوع الفاتورة: ضريبية/غير ضريبية/ضريبية مبسطة مع إمكانية تثبيت النوع وتعديله (إضافة افتراضي على مستوى النظام + تخصيص افتراضي لكل مستخدم/فرع).
- [x] شروط وأحكام/ضمان قابلة للحفظ والتعديل والحذف والطباعة (إدارة قوالب متعددة وربطها بالإعدادات لاختيار نص للطباعة).
- [x] مقاسات طباعة: A4/A5/فاتورة نقطية (كاشير).
- [x] منع تعديل أو حذف الفاتورة بعد الترحيل. (فواتير المبيعات تُقفل تلقائياً وتمنع التعديل أو الحذف بعد الحفظ؛ يسمح فقط بتحديث حالة/مبلغ الدفع)
- [x] منع تعديل التاريخ والوقت. (يتم فرض تاريخ/وقت السيرفر عند الحفظ والمرتجع)
- [x] بحث متقدم برقم/تاريخ/عميل/مندوب/صنف.
- [x] اختصارات إضافة عميل/صنف/مندوب من شاشة الفاتورة (روابط إضافة سريعة تفتح في تبويب جديد للمبيعات والمشتريات).

### فواتير المشتريات
- [x] نفس سلوك الوحدات والتحذير عند التكرار (مع حفظ معامل التحويل لكل سطر وتطبيقه على المخزون).
- [x] حفظ رقم فاتورة المورد كمرجع مع أرشفة صورة.
- [x] أنواع دفع: أجل (مورد إلزامي)، نقدي، شبكة، أو مزيج مع عدة بنوك (مدفوعات المشتريات تدعم نقد + عدة بطاقات مع تحقق المجموع).
- [x] مركز تكلفة (مورد + مندوب) قابل للإضافة快捷. (اختيار مندوب لكل فاتورة مشتريات مع مركز تكلفة اختياري يُعبّأ تلقائياً باسم المندوب عند اختياره)
- [x] احتساب الضريبة شامل/غير شامل.
- [x] حقل ملاحظات.
- [x] عند اختيار مورد افتراضي تظهر حقول اسم ورقم جوال (حقلان اختياريان مع تفعيل مورد نقدي/افتراضي).
- [⚠️] إدخال رقم تشغيله/باتش مع العدد والتاريخ. (تمت إضافة إدخال وتشغيله في المشتريات، تحتاج إدارة/بحث أوسع)
- [x] حفظ رقم فاتورة المورد كمرجع مع أرشفة صورة.
- [x] إمكانية تعديل الفاتورة.
...
## 9) نقاط البيع (POS)
- [x] تصميم فواتير حرارية مناسب يتضمن بيانات المستخدم، عنوان الفاتورة (ضريبية مبسطة)، بيانات العميل، ملاحظة، شروط/سياسة استبدال/إرجاع.
