...
class ClientStatusReportPdfController extends Controller
{
    public function pdf(Request $request)
    {
        ...
        $transactions = $this->queryTransactions(3, $fromDate, $toDate, $invoiceNo, $amountMin, $amountMax);
        $companyIds = $transactions->pluck('company_id')->filter()->unique()->values();

        $companiesQuery = Company::query()->where('group_id', 3);
        if ($status === 'active') {
        ...
        $lastTransactions = $this->lastTransactions(3, $companies->pluck('id')->all());
        ...
        $pdf = SnappyPdf::loadView('reports/client-status-report', $payload)
